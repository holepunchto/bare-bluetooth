...
class Central extends EventEmitter {
  ...
  startScan(serviceUUIDs, opts = {}) {
    ...
    // BlueZ answers a second StartDiscovery on the same connection with
    // InProgress. Mirror stopScan and make a redundant call a no-op rather than
    // an error, so a caller retrying a scan does not have to track this itself.
    if (this._scanning) return

    this._allowDuplicates = opts.allowDuplicates === true
    this._filter = uuidFilter(serviceUUIDs)
    this._seen.clear()

    // BlueZ announces a device with InterfacesAdded only the first time it ever
    // sees it, and keeps unbonded devices in its object tree for minutes after
    // a scan ends. Without seeding from that cache, scanning for a device you
    // already scanned for once silently never finds it, even though it is
    // sitting right there advertising. Seed the tree here so those devices have
    // RSSI watchers, and report them when an advertisement actually arrives.
    this._watchKnown()
  ...
  _ondevice(device) {
    ...
    // A device announced by InterfacesAdded is being advertised right now, so
    // report it without waiting for an RSSI update.
    this._seen.add(device.address)
  ...
  _watchKnown() {
    ...
    try {
      ...
      // Enumeration is an optimisation on top of discovery, so a backend too
      // old to support it should degrade rather than fail the scan.
      this.emit('error', err)
      return
    } finally {
      this._enumerating = false
    ...
    for (const device of this._adapter.devices.values()) this._watch(device)
  }

  _watch(device) {
    if (this._watched.has(device)) return
    this._watched.add(device)

    // Later advertisements from a device BlueZ already knows arrive as RSSI
    // property changes rather than InterfacesAdded. Waiting for one is what
    // distinguishes a device that is present from a stale cache entry.
    device.on('rssi', () => {
      if (!this._scanning) return

      if (this._seen.has(device.address)) {
        if (!this._allowDuplicates) return
      } else {
        this._seen.add(device.address)
      }

      if (!this._matches(device)) return

      this.emit('discover', this._discovered(device))
    })
  }

  // BlueZ applies the discovery filter itself, but it does so against the
  // advertisement rather than the cached record, so check here too.
  _matches(device) {
    ...
    for (const uuid of uuids) {
      if (this._filter.has(normalizeUuid(uuid))) return true
    }
