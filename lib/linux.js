...
class Characteristic {
  ...
  _subscribe(ondata) {
    if (!this._ondata) this._native.on('data', (this._ondata = ondata))
    return this._native.startNotify()
  }

  _unsubscribe() {
    if (this._ondata) this._native.off('data', this._ondata)
    this._ondata = null
    return this._native.stopNotify()
  }

  _update(data) {
    if (!this._native.registered) return false
    this._native.value = data
    return true
  }
...
class Peripheral extends EventEmitter {
  constructor(device) {
    ...
    this._listeners = {
      servicesResolved: this._onresolved.bind(this),
      channelOpen: this._onchannelopen.bind(this),
      connected: this._onconnected.bind(this),
      error: this._onerror.bind(this)
    }
    for (const [event, fn] of Object.entries(this._listeners)) device.on(event, fn)
  }
  ...
  subscribe(characteristic) {
    characteristic
      ._subscribe((data) => this.emit('notify', characteristic, data))
      .then(
  ...
  _onconnected(connected) {
    ...
    for (const [event, fn] of Object.entries(this._listeners)) this._device.off(event, fn)
    this._waiting = []
...
class Server extends EventEmitter {
  ...
  addService(service) {
    ...
    this._queue(() => this._register().then(() => this.emit('serviceAdd', service.uuid)))
  }
  ...
  updateValue(characteristic, data) {
    return characteristic._update(data)
  }
  ...
  startAdvertising(opts = {}) {
    ...
    this._queue(() => this._adapter.registerAdvertisement(advertising))
  }
  ...
  stopAdvertising() {
    ...
    const advertising = this._advertising
    this._advertising = null
    this._queue(() => this._adapter.unregisterAdvertisement(advertising))
  }
  ...
  _attach(characteristic) {
    ...
    native.read = (options) => {
      if (this.listenerCount('readRequest') === 0) return native.value.subarray(options.offset)
