...
class Central extends EventEmitter {
  ...
  _ondevice(device) {
    ...
    device.on('name', () => {
      this._seen.delete(device.address)
  ...
  _discover(device) {
    ...
    if (!this._allowDuplicates) {
      if (this._seen.has(device.address)) return
...
// respondToRequest needs BlueZ to hand ReadValue/WriteValue to JS before it
// answers. bare-bluetooth-linux replies from its own cache today, so a server
// here could not honour the contract. Refuse rather than half-implement it.
