...
// CoreBluetooth reports an RSSI of 127 when the reading is unavailable, which
// is not a signal strength.
class Discovered {
  ...
  get serviceData() {
    return this.peripheral.serviceData
  }
...
class Central extends EventEmitter {
  ...
  startScan(serviceUUIDs, opts = {}) {
    this._native.startScan(serviceUUIDs, { allowDuplicates: opts.allowDuplicates })
  }
  ...
  connect(discoveredPeripheral) {
    this._native.connect(discoveredPeripheral._native.peripheral)
  }
  ...
  _ondiscover(discovered) {
    this.emit('discover', new DiscoveredPeripheral(new Discovered(discovered)))
  }
