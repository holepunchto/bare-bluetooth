...
class Peripheral extends EventEmitter {
  constructor(nativePeripheral) {
    ...
    this._native.on('servicesDiscover', function (services, error) {
      ...
      self.emit(
        'servicesDiscover',
        services.map((s) => self._wrapService(s)),
        null
      )
    })

    this._native.on('characteristicsDiscover', function (service, chars, error) {
      const wrappedService = service ? self._wrapService(service) : null
      if (error || !chars) {
        self.emit('characteristicsDiscover', wrappedService, null, error)
        return
      }
      const wrappedChars = chars.map((c) => self._wrapCharacteristic(c))
    ...
    this._native.on('read', function (char, data, error) {
      self.emit('read', char ? self._wrapCharacteristic(char) : null, data, error)
    })

    this._native.on('write', function (char, error) {
      self.emit('write', char ? self._wrapCharacteristic(char) : null, error)
    })

    this._native.on('notify', function (char, data, error) {
      self.emit('notify', char ? self._wrapCharacteristic(char) : null, data, error)
    })

    this._native.on('notifyState', function (char, isNotifying, error) {
      self.emit('notifyState', char ? self._wrapCharacteristic(char) : null, isNotifying, error)
    })
  ...
  _wrapService(native) {
    ...
    if (!wrapped) {
      wrapped = Service.from(native)
      this._services.set(native, wrapped)
    }
    return wrapped
  }

  _wrapCharacteristic(native) {
    let wrapped = this._chars.get(native)
    if (!wrapped) {
      wrapped = Characteristic.from(native)
